...
def carregar_base(_dummy=None):
    """Carrega BASEBOT.csv do Supabase Storage. Fallback para arquivo local."""
    import io
    try:
        r = requests.get(SUPABASE_STORAGE_URL, timeout=60)
        if r.status_code == 200:
            df = pd.read_csv(io.StringIO(r.content.decode("utf-8-sig")),
                             sep=";", dtype=str, low_memory=False)
            return _processar_df(df)
        else:
            st.warning(f"Supabase Storage retornou {r.status_code} — tentando arquivo local...")
    except Exception as e:
        st.warning(f"Erro ao carregar do Supabase Storage: {e}")
